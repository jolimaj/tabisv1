    ...
    <div class="flex justify-start mb-8 max-w-3xl">
      <h1 class="text-3xl font-bold">
        ...
        <span class="text-indigo-400 font-medium">/</span>
          {{ user.admin ? 'Admin' :  `${form.first_name} ${form.last_name}` }}
      </h1>
    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        <div class="flex flex-wrap -mb-8 -mr-6 p-8">
          ...
          <text-input v-model="form.email" :error="form.errors.email" class="pb-8 pr-6 w-full lg:w-1/2" label="Email" :disable="pageName.includes('profile')"/>
          ...
          <!-- <select-input v-model="form.admin" :error="form.errors.admin" class="pb-8 pr-6 w-full lg:w-1/2" label="Role">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!user.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy" :hidden = "pageName.includes('profile')">Delete User</button>
